...
def run_tests(update=False):
    files = [
        f
        for f in os.listdir(".")
        if f.startswith("cover_story_") and f.endswith(".jpeg")
    ]
    files.sort()
    ...
    for filename in files:
        ...
        if update:
            ...
            if not compare_rects(baseline["board_rect"], current_v["board_rect"]):
                print(
                    f"  FAILED: Board Rect mismatch. Expected {baseline['board_rect']}, got {current_v['board_rect']}"
                )
                match = False
            if baseline["num_colors"] != current_v["num_colors"]:
                print(
                    f"  FAILED: Color count mismatch. Expected {baseline['num_colors']}, got {current_v['num_colors']}"
                )
                match = False
            if baseline["grid"] != current_v["grid"]:
                print("  FAILED: Grid layout mismatch.")
                match = False
            if baseline["total_blocks"] != 25:
                print(
                    f"  WARNING: Total blocks is {current_v['total_blocks']}, expected 25 for a full board."
                )
            ...
            if baseline["total_blocks"] != current_v["total_blocks"]:
                print(
                    f"  FAILED: Block count mismatch. Expected {baseline['total_blocks']}, got {current_v['total_blocks']}"
                )
                match = False
            if baseline["has_solution"] != current_v["has_solution"]:
                print("  FAILED: Solver status mismatch.")
                match = False
